...
# Loop through each scene directory in ./data/DROID/
for scene_dir in ./data/DROID/*/; do
    if [ -d "$scene_dir" ]; then
        # Extract scene_name from the directory name
        scene_name=$(basename "$scene_dir")
        # If scene_name doesn't start with "droid", skip the scene
        if [[ ! "$scene_name" =~ ^droid ]]; then
        ...
        object_position="data/DROID/${scene_name}/masks/transformations_new.json"
        mask_folder="./data/DROID/${scene_name}/bounding_boxes_and_masks"
        rgb_folder="./data/DROID/${scene_name}/images0"
        pcd_folder="./data/DROID/${scene_name}/pcds"
        scene_video="./data/DROID/${scene_name}/video.mp4"
        object_mask_video="./data/DROID/${scene_name}/mask.mp4"
        robot_mask_video="./data/DROID/${scene_name}/robot_mask.mp4"
        output_folder="./data/input_mask/${scene_name}/"

        # **Update configs/DROID_config.yaml dynamically for the current scene**
        cat <<EOT > configs/DROID_config.yaml
...
EOT
        ...
        echo "Updated configs/DROID_config.yaml for scene: $scene_name"
        ...
        if [ "$use_bypass" == "true" ]; then
            ...
            echo "Saved masks in data/DROID/$scene_name/masks/"
        ...
        if [[ "$use_bypass" == "false" && "$use_molmo" == "false" ]]; then
            read w h <<< $(ffprobe -v error -select_streams v:0 -show_entries stream=width,height \-of csv=p=0:s=x ./data/DROID/$scene_name/images0/0.jpg | tr 'x' ' ')
            mkdir -p ./data/DROID/$scene_name/images0/super_res/
            cp ./data/DROID/$scene_name/images0/1.jpg ./data/DROID/$scene_name/images0/super_res/
            conda activate invsr
            cd InvSR
            python3 inference_invsr.py \
                -i ../data/DROID/${scene_name}/images0/super_res/ \
                -o ../data/DROID/${scene_name}/images0/super_res/ \
                --num_steps 2
            cd ..
            ffmpeg -i ./data/DROID/${scene_name}/images0/super_res/1.png \
                -vf "scale=${w}:${h}" \
                -y ./data/DROID/${scene_name}/images0/super.png
            rm -rf ./data/DROID/${scene_name}/images0/super_res/
        fi
        ...
        if [ "$use_bypass" == "false" ]; then
            ...
            if [ "$use_molmo" == "false" ]; then
                ...
                cp -r ./data/DROID/$scene_name/input_mask ./data/input_mask/$scene_name
                rm -rf ./data/DROID/$scene_name/input_mask
                echo "Saved masks in data/DROID/$scene_name/masks/"
                conda deactivate
            fi
        fi
        ...
        if [[ "$use_bypass" == "false" && "$use_molmo" == "false" ]]; then
            ...
            conda activate invsr
            cd InvSR
            python3 inference_invsr.py -i ../data/DROID/$scene_name/masks/ -o ../data/DROID/$scene_name/masks/ --num_steps 2
            src="../data/DROID/$scene_name/masks"
            out_dir="../data/DROID/$scene_name/masks"
            echo "Resizing images in $src to ${w}x${h}..."
            for img in "../data/DROID/$scene_name/masks/"*.png; do
                [ -e "$img" ] || continue
                filename=$(basename "$img")
                tmpfile=$(mktemp --suffix=".${filename##*.}")
                ffmpeg -i "$img" -vf "scale=${w}:${h}" -y "$tmpfile"
                mv "$tmpfile" "../data/DROID/$scene_name/masks/$filename"
                echo "Resized $img to ${w}x${h} and saved to ../data/DROID/$scene_name/masks/$filename"
            done
            conda activate invsr
            python3 inference_invsr.py -i ../data/DROID/$scene_name/masks/cropped/ -o ../data/asset_images/$scene_name/enhanced/ --num_steps 2
            cd ../
            conda deactivate
        fi
        ...
        conda activate match
        export PYTHONPATH=$(pwd)
        CONFIG_FILE="configs/config_DROID.yaml"
        ...
        export PYTHONPATH=$(pwd)
        python3 src/demo/droid_demo.py --config "./configs/DROID_config.yaml"
        conda deactivate
